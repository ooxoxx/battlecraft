...
export default defineEventHandler(async (event) => {
  ...
  const { className, specName } = getQuery(event) as { className: string, specName: string }

  const { wcl } = useRuntimeConfig(event)

  const client = await getClient(wcl)
  const data = await client.query({ query: CHARACTER_RANKING, variables: { id: 12660, className, specName } })
